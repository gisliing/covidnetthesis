...
def covidnet_train(epochs, learning_rate, batch_size, weightspath, metaname, ckptname,trainfile,
    ...
    with tf.Session() as sess:
        ...
        eval(sess, graph, testfiles, os.path.join('/p/project/joaiml/ingolfsson1/COVID_Net/data','test'),
             in_tensorname, out_tensorname, input_size, mapping)
...
for i in recvbuf:
    ...
    batch_size = 8
    weightspath = '/p/project/joaiml/ingolfsson1/COVID_Net/models/COVIDNet-CXR-Gisli'
    metaname = 'model.meta'
    ckptname = 'model-8485'
    n_classes = 3
    trainfile = '/p/project/joaiml/ingolfsson1/COVID_Net/labels/train_COVIDx8A.txt'
    testfile = '/p/project/joaiml/ingolfsson1/COVID_Net/labels/test_COVIDx8A.txt'
    class_weights = [1., 1., 1.]
    #covid_percent = 0.14
    covid_percent = 0.5
    input_size = 224
    ...
    in_tensorname = 'input_1:0'
    logit_tensorname = 'norm_dense_1/Softmax:0'
    label_tensorname = 'norm_dense_1_target:0'
    weights_tensorname = 'norm_dense_1_sample_weights:0'

    t = time.time()
    loss = covidnet_train(epochs, learning_rate, batch_size, weightspath, metaname, ckptname, trainfile,
                          testfile, class_weights, covid_percent, input_size, out_tensorname, in_tensorname,
                          logit_tensorname, label_tensorname, weights_tensorname)
    t = time.time() - t
